...
def plot_fn(
    ...
    if min_points is not None:
        ...
        for i, point in enumerate(min_points):
            min_points[i][0] = {"x_min": x_range[0], "x_max": x_range[1]}.get(
                point[0], point[0]
            )
            min_points[i][1] = {"y_min": y_range[0], "y_max": y_range[1]}.get(
                point[1], point[1]
            )
...
def plot_fn_with_points(
    ...
    n_points=100,
    min_points: (
        list[float | str, float | str] | list[list[float | str, float | str]] | None
    ) = None,
    log_scale=False,
    return_fig=False,
    filename: str | None = None,
    ...
    for (xys, name, params), color in zip(points, px.colors.qualitative.Set1):  # type: ignore
        ...
        fig.add_trace(  # type: ignore
            go.Scatter(
                ...
                text=[
                    f"step {i}<br>x={x[i]:.4f}<br>y={y[i]:.4f}<br>z={z[i]:.4f}"
